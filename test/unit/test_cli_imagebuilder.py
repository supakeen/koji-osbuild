...
@ImageBuilderPluginTest.load_plugin("cli")
class TestCliPlugin(ImageBuilderPluginTest):
    ...
    def mock_options(*, quiet=False):
        options = flexmock(quiet=quiet, weburl="http://osbuild.org/")
        return options
    ...
    def mock_session_add_valid_tag(session):
        ...
        session.should_receive("getBuildTarget").with_args("target").and_return(
            build_target
        ).once()

        session.should_receive("getTag").with_args(
            build_target["dest_tag"]
        ).and_return(tag_info).once()
    ...
    def test_basic_invocation(self):
        ...
        with self.assertRaises(SystemExit) as cm, contextlib.redirect_stderr(f):
            self.plugin.handle_imagebuilder_build(None, None, argv)
        ...
        session.should_receive("imageBuilderBuild").with_args(
            *expected_args, opts=expected_opts
        ).and_return(task_id).once()

        session.should_receive("logout").with_args().once()

        session.should_receive("getTaskResult").with_args(task_id).and_return(
            task_result
        ).once()

        setattr(self.plugin, "kl", koji_lib)
        r = self.plugin.handle_imagebuilder_build(options, session, argv)
        self.assertEqual(r, 0)

    def test_customizations_options(self):
        with tempfile.TemporaryDirectory() as tmpdir:

            customizations = {"packages": ["emacs"]}
            ...
            session.should_receive("getTaskResult").with_args(
                task_id
            ).and_return(task_result).once()
    ...
    def test_ostree_options(self):
        ...
        argv = [
            ...
            "ostree/$arch/staging",
            "--ostree-ref",
            "ostree/$arch/production",
            "--ostree-url",
            "https://osbuild.org/repo",
        ]
    ...
    def test_repo_package_sets(self):
        ...
        argv = [
            ...
            "alpha",
            "--repo",
            "https://third.repo",  # NB: no `--repo-package-set`
            "--release",
            "20200202.n2",
        ]
        ...
        expected_opts = {
            ...
            "repo": [
                ...
                {"baseurl": "https://second.repo", "package_sets": ["alpha"]},
    ...
    def test_target_check(self):
        ...
        with self.assertRaises(koji.GenericError):
            self.plugin.handle_imagebuilder_build(None, session, argv)
