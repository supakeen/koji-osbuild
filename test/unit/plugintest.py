...
class OSBuildPluginTest(unittest.TestCase):
...
class ImageBuilderPluginTest(unittest.TestCase):
    """Base class for Plugin tests
    ...
    def _load_plugin(self):
        ...
        root = os.getenv("GITHUB_WORKSPACE", os.getcwd())
        haystack = os.path.join(root, "plugins", plugin_type, "imagebuilder.py")
        spec = importlib.util.spec_from_file_location("imagebuilder", haystack)
        assert spec, f"Could not find '{plugin_type}' plugin"
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        return module
