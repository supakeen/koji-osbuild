...
def parse_args(argv):
    usage = (
        "usage: %prog image-builder-build [options] <name> <version> "
    ...
    parser.add_option(
        ...
        type=str,
        help=(
            "Specify a repo that will override the repo used to install "
    ...
    parser.add_option(
        ...
        type=str,
        action="callback",
        callback=parse_repo_package_set,
        help=(
            "Specify the package sets for the last repository. "
    ...
    parser.add_option(
        "--skip-tag", action="store_true", help="Do not attempt to tag package"
    )
    parser.add_option(
        "--wait",
        action="store_true",
    ...
    if len(args) < 5:
        parser.error(
            "At least five arguments are required: a name, "
...
def check_target(session, name):
    ...
    if not tag:
        raise koji.GenericError(
            f"Unknown destination tag: {target['dest_tag_name']}"
        )


# pylint: disable=too-many-branches
@export_cli
def handle_imagebuilder_build(options, session, argv):
    "[build] Build images via image-builder"
    args = parse_args(argv)

    name, version, arch, target = (
        args.name,
        args.version,
        args.arch,
        args.target,
    ...
    task_id = session.imageBuilderBuild(
        name, version, distro, image_type, target, arch, opts=opts
    )
